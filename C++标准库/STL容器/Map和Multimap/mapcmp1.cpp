...
typedef map<string,string,RuntimeStringCmp> StringStringMap; //实例化map类型，以RuntimeStringCmp的默认状态进行string的排序

// function that fills and prints such containers
void fillAndPrint(StringStringMap& coll); //赋值并打印这个map的所有内容

int main()
{
    // create a container with the default comparison criterion
    StringStringMap coll1; //排序准则的默认状态创建map
    fillAndPrint(coll1); //赋值并打印这个map的所有内容

    // create an object for case-insensitive comparisons
    RuntimeStringCmp ignorecase(RuntimeStringCmp::nocase); //创建一个排序准则状态为大小写无关的函数对象

    // create a container with the case-insensitive comparisons criterion
    StringStringMap coll2(ignorecase); //用这个大小写无关的对象，创建map
    fillAndPrint(coll2); //赋值并打印这个map的所有内容
}
