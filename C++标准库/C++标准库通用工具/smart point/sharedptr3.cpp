...
class SharedMemoryDetacher
{
  public:
    void operator () (int* p) {
        ...
        if (shm_unlink("/tmp1234") != 0) { //卸载共享内存
...
{
    void* mem;
    int shmfd = shm_open("/tmp1234", O_CREAT|O_RDWR, S_IRWXU|S_IRWXG); //创建共享内存对象
    if (shmfd < 0) {
        throw std::string(strerror(errno));
    }
    if (ftruncate(shmfd, num*sizeof(int)) == -1) { //设置共享内存大小
        throw std::string(strerror(errno));
    }
    mem = mmap(nullptr, num*sizeof(int), PROT_READ | PROT_WRITE, 
               MAP_SHARED, shmfd, 0); //创建共享内存
    if (mem == MAP_FAILED) {
        throw std::string(strerror(errno));
    }
